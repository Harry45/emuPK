...
def maximise(x_train=None, y_train=None):
    '''
    ...
    glm_module.regression_prior(lambda_cap=st.lambda_cap)
    ...
    gp_module = sgp.GP(theta=x_train, y=y_train, var=st.var, order=st.order, x_trans=st.x_trans, y_trans=st.y_trans,
